    ...
    <div onClick={() => onClick(book)}
      ...
      <div style={{ width: "100%", aspectRatio: "2/3", overflow: "hidden", borderRadius: 6, background: "#2C1A0E" }}>
        {book.book?.thumbnail_url
          ...
          : <ColorCover color={book.book?.cover_color || "#C4956A"}
              title={book.book?.title || ""} author={book.book?.author || ""} size={88} />
        }
  ...
  return (
    <div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: 340,
      ...
      <div style={{ height: 200, background: "#0A0604", display: "flex", alignItems: "center",
        ...
        {book.book?.thumbnail_url ? (
          <>
            ...
            <img src={book.book.thumbnail_url} alt={book.book?.title}
              style={{ height: 150, width: "auto", borderRadius: 6,
      ...
      <div style={{ padding: "16px 20px", flex: 1, overflowY: "auto" }}>
        ...
        <div style={{ fontSize: 12, color: "#8A7A6A", marginBottom: 16 }}>{book.book?.author}</div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input type="range" min={0} max={pageCount || 100} value={page}
            onChange={e => setPage(Number(e.target.value))}
            style={{ width: "100%", accentColor: "#E8934A" }} />
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
            <span style={{ fontSize: 11, color: "#6A5A4A" }}>0</span>
            <span style={{ fontSize: 12, color: "#E8D5B0", fontWeight: 600 }}>{page} / {pageCount} ページ</span>
          </div>
        </div>
...
export default function BookshelfDashboard() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      const { data: profile } = await supabase
        .from("profiles").select("coins").eq("id", uid).single();
      if (!profile) {
  ...
  const handleBookUpdate = async (updated) => {
    ...
    await updateUserBook(updated.id, {
      ...
      notes: updated.notes,
      status: updated.current_page >= (updated.book?.page_count || 0) ? "completed" : "reading",
    });
    if (pagesAdded > 0) {
    ...
    <div style={{ minHeight: "100vh", background: "#0D0703", color: "#E8D5B0",
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", minHeight: "100vh" }}>
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          {/* Header */}
          <header style={{ padding: "20px 28px 16px", display: "flex", alignItems: "center", gap: 16,
            borderBottom: "1px solid rgba(196,168,130,0.08)", background: "rgba(10,6,3,0.6)", backdropFilter: "blur(10px)" }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: "#E8D5B0", letterSpacing: -0.5 }}>わたしの本棚</div>
          ...
          <div style={{ flex: 1, display: "flex" }}>
            ...
            <div style={{ flex: 1, padding: "24px 28px", overflowY: "auto", minWidth: 0 }}>
              ...
              {!loading && filteredBooks.length === 0 && (
                <div style={{ textAlign: "center", padding: "60px 0" }}>
                  ...
                  <div style={{ fontSize: 14, color: "#6A5A4A", marginBottom: 16 }}>まだ本が登録されていません</div>
            ...
            <aside style={{ width: 240, borderLeft: "1px solid rgba(196,168,130,0.08)",
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 11, color: "#6A5A4A", letterSpacing: 1,
                  textTransform: "uppercase", marginBottom: 12 }}>マイペット</div>
                <div onMouseEnter={handlePetHover}
                  style={{ width: 120, height: 120, margin: "0 auto",
                    animation: "floatPet 3s ease-in-out infinite",
                    filter: "drop-shadow(0 8px 24px rgba(196,149,106,0.25))", cursor: "pointer" }}>
              ...
              <div style={{ borderTop: "1px solid rgba(196,168,130,0.08)", paddingTop: 16 }}>
                <div style={{ fontSize: 11, color: "#6A5A4A", letterSpacing: 1,
                  textTransform: "uppercase", marginBottom: 12 }}>読書記録</div>
                {[
